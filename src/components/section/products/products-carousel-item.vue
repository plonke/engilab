<template>
  ...
  >
    ...
    <div class="flex">
      <SharedDivider direction="vertical" class="order-2" />

      <div
        v-for="(marketplace, index) in product.marketplaces"
        class="corner -gap-[10px] -md:gap-[10px] group relative flex basis-1/2 cursor-pointer flex-col -items-center py-[20px] transition-all hover:bg-light-200"
        :style="{ order: index * 2 }"
      >
        <div
          class="mb-[5px] flex w-full gap-[0px] font-mono text-subtitle-14 md:mb-[5px]"
        >
          <div>
            {{ marketplace.name }}
          </div>
          ...
          <div
            class="absolute right-[20px] top-[20px] size-[20px] md:size-[20px]"
          >
        ...
        <div
          class="relative mb-[15px] overflow-hidden font-mono text-body-14 opacity-50 md:text-body-14"
        >
          <div>
            {{ marketplace.reviews }}
          </div>
